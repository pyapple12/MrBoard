...
- **三路代理 + 行为验证为强制项**（第 10 轮起固化）：单路审计无法暴露跨模块交互缺陷（如节流缓存破坏预警去重，仅三路交叉 + 实测抓出）
- **修复验证独立化**（第 10 轮教训）：探针结构性样例必须来自真实数据快照或独立证据（现网 api.json 片段/官方 schema 字段名），禁止手写与实现同源的 mock 自证（C0.1 字段名 "pricing" vs "cost" 漏网根因）；手写 mock 仅用于行为验证
- **批次完成加修复验收**：每批修复完成后、全量回归前，逐项对照原问题做反向验证（删键报错/换序报错/坏模板报错），防"修复引入新缺陷"流入下轮（C0.1/C0.8/C0.6 均为上轮修复自身缺陷）

### 5.3 重构类任务的执行教训（A017/PL006 接口层实测，2026-08-25）

> 背景：PL006 将同步任务改造为 QThreadPool+信号异步回传，实施耗时远超预估。
> 根因复盘出三类坑与对应规则，已固化进 AGENTS.md「操作注意」，此处记录完整推理。

**坑一：mock.patch 生命周期与 worker 不重叠**——`with mock.patch(...)` 在任务提交后立即退出恢复原值，而 worker 线程稍后才执行，导致走真实 browser_creds（启动真 Chrome/CDP 连接重试）挂起 120s+。表象（挂起/真实浏览器启动）与根因相距甚远，走了"补 mock 列表→换 patch 目标→怀疑线程调度"三轮弯路。正确姿势：`with` 块必须包住"提交+轮询等待+断言"全程。

**坑二：QRunnable wrapper 被 GC 触发 0xC0000409 fail-fast 秒杀**——QThreadPool.start 后 Python wrapper 无引用即被回收，C++ 悬空。fail-fast 截断 stderr，faulthandler 都来不及输出。修复三件套：`_live_tasks` 集合持执行中引用 + 完成后转 `deque(maxlen=16)` 保最近引用防悬空 + setAutoDelete(False) 由 Python 全权管理。

**坑三：run_all_verify 的 timeout=120 强杀误判为断言失败**——s7 批量跑偶超 120s 被 [TIMEOUT] 杀掉，先按"断言失败"查 FAIL 行（没有），后才发现汇总器配置。看 exit code 特征（无 FAIL 行但非零退出）应第一时间怀疑超时而非断言。

**元教训三条**：

1. 挂起/秒杀类问题第一动作是 faulthandler 线程栈转储写文件，禁止猜测性修补（盲猜三轮 vs 栈转储一次定位的实证差距）
2. 异步化重构的测试探针无法在 TDD 前置阶段覆盖时序特性——接受"运行时炸出来再固化模板"的现实，但炸出的第一个坑必须立即沉淀为可复用模板
3. 工具纪律违规（PowerShell Replace 替代 edit）两次引发结构损坏，省下的操作时间被事故排查成倍吃掉——纪律不是形式主义是成本核算
