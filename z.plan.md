...
## 附录 A021：QML 前端 UI 设计审计报告（第21轮，2026-08-28）

> 范围：ui/qml/ 全部程序文件（main.qml / UsagePage.qml / DataPage.qml / theme/Theme.qml / effects/CardShadow.qml）+ 对应 Qt 官方 qt-ui-design skill 审计清单（桌面目标：Motion/Typography/Keyboard/Colour/Localisation）
> 方式：首次引入 Qt 官方 agent-skills（qt-ui-design 设计审计清单）对 QML 演示版做设计合规审计——非代码正确性审计，聚焦视觉/可用性/无障碍
> 结果：**P 级 6 条（Critical 3 / Warning 3，无 Opportunity 提升）/ 参考级观察项 2 条维持豁免（用户 2026-08-28 复核确认）**
> 状态：📌 待修复（P 系列任务清单见 x.progress.md）

### 零、上轮修复复核清单

本轮为新领域审计（QML UI 设计审计），无上轮同领域复核项；A020 代码整改（O 系列）已随 V0.2.5.5 完成，复核见 A020 自身。
...
| 文件:行号                                       | 类型 | 描述                                                                                                                                                                                   | 建议                                                                                                                   | 性质           | 影响面      |
| ----------------------------------------------- | ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | -------------- | ----------- |
| ui/qml/UsagePage.qml:190/263/291 + DataPage.qml | ⑩    | 正文字号全线 <16px（卡片标题 12/进度条标签 12/配额账户 14/表格 12/动态正文 12），且全用 font.pixelSize 不随 OS 字号缩放——桌面监控信息窗核心价值是一眼读数字，12px 在 60cm 视距可读性差 | 建 TypeScale 单例（modular scale：12/16/21/28 档）+ 正文改 font.pointSize；字号节奏随本次一并归位（原 Warning 4 并入） | 新增（雏形期） | 显示层      |
| ui/qml/theme/Theme.qml:17                       | ⑩    | chunkWarn 黄 #FFB020 在白底（cardBg #FFFFFF）对比度约 1.8:1，低于 UI component ≥3:1 要求——60~80% 进度区间视觉弱                                                                        | 黄色调深至对比度 ≥3:1（如 #E58E00 附近），同批核对三色白底/浅底对比度达标                                              | 新增（雏形期） | 显示层      |
| ui/qml/UsagePage.qml:28-61                      | ⑩    | 无 reduced-motion 路径：背景粒子持续动画（emitRate 8/lifeSpan 3500）+ 入场 300ms 不可关——skill 要求桌面 UI 提供动画开关，关闭时瞬时过渡                                                | Theme 单例增 reducedMotion 属性（QSettings 或代码开关），粒子/过渡绑定之；关闭时粒子停发、opacity 直达 1               | 新增（雏形期） | 显示层/性能 |

**Warning（3 条，入 P1 组）**

| 文件:行号                           | 类型 | 描述                                                                                                                                                                         | 建议                                                                                      | 性质           | 影响面          |
| ----------------------------------- | ---- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- | -------------- | --------------- |
| ui/qml/Theme.qml（缺）+ 两页        | ③    | 颜色未完全 token 化：边框 #E0E0E0×6（UsagePage:176/206/234、DataPage:147/184/235）、斑马纹 #F4F4F4（DataPage:183）、圆角 radius 8 vs 6 不一致（UsagePage:174、DataPage:234） | Theme 单例补 borderSubtle/rowStripe token + radius 统一（8）；两页硬编码色/圆角改引用单例 | 新增（雏形期） | 显示层/配置体系 |
| ui/qml/UsagePage.qml + DataPage.qml | ⑩    | 无 Accessible 属性：自定义 Text/进度条标签均无 Accessible.name/role——键盘可达与读屏不可达（skill 要求每个交互元素键盘 + 读屏可达）                                           | 交互元素（添加账户按钮/导航项/进度条）补 Accessible.name/role/description                 | 新增（雏形期） | UI 交互/无障碍  |
| ui/qml/UsagePage.qml:310-322        | ⑩    | 饼图扇区纯颜色分级无图例且 legend.visible=false——"颜色永不单独承载状态"，色弱用户无法区分扇区含义                                                                            | 图例开/补百分比标签（扇区外 label 或列表），分级色 + 文案双编码                           | 新增（雏形期） | 显示层          |

### 二、参考级观察项（全部维持豁免，用户 2026-08-28 复核确认）

| 编号 | 内容                                             | 回落理由                                                               |
| ---- | ------------------------------------------------ | ---------------------------------------------------------------------- |
| Q-7  | 五张卡片仅数字+小标题，无上下文提示（增量/环比） | 监控信息窗信息密度优先，上下文提示属增强非缺陷，待真实数据接入后按需加 |
| Q-8  | 表格/列表无空态视觉插图，仅文字占位              | 空态文案已达标（data_empty_text/releases_empty），插图属锦上添花       |

### 三、亮点

- 数据流分层正确：context property 注入（阈值/文案/模型）+ Theme 单例自持色板 + 格式化对齐 qt6 口径，为 QML 接入真实业务（P27）铺平了契约
- 卡片数值/饼图/进度条三处分级色共用同一 colorForPercent 分支，口径一致
- FluInfoBar + 缓存标注 + 保留旧视图落实了"不崩溃/不阻塞/有提示"的错误策略（PL008.9.b）
