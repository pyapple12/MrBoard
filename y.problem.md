...
### P22. 测试专用接口的去留评估（增强候选）

- **状态**：📌 待评估（2026-08-12 C2 审计评估后用户提出）
- **来源**：2026-08-12 用户："对于这种特供测试的内容，后续还是需要考虑删除"
- **现状**：`_RemainingPieChart.used_percent()` getter 生产代码零调用，仅 verify 测试（verify_v0809_4/verify_v3a1 等）用于断言饼图状态（如 `used_percent() == 80`）——"外部读取/测试用"注释已声明
- **考虑点**：
  - 保留：控件公共读取接口是标准设计（类比 `QProgressBar.value()`），测试使用证明有实际消费者
  - 删除：需同步修改引用它的测试（改用其他方式验证饼图渲染状态），删除后控件 API 缩减
  - 同类待排查：是否还有其他"仅测试消费"的接口（如 `_quota_bars`/`_quota_reset` 等测试直接访问的内部状态）
- **触发条件**：下次审计或测试架构调整时评估（若删除，连带更新 verify_v0809_4/verify_v3a1 等断言）
