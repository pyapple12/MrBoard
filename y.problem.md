...
- **背景**：首轮审计（S7 前置）后经历 S8 大改、CDP 修复与 V0.08 改造（P2-P8），代码已大幅变化，需二次全量审计
- **审计范围**：
  1. **硬编码排查**：V0.08 后残留硬编码（已知：P2 已解决；待查：中文文案/表头/维度标签、错误消息、魔法数字等）
  2. **代码规范错误**：对照 AGENTS.md（函数 `#` 注释、文件尾说明区、命名、import 顺序、行宽、类型注解、异常处理）
  3. **代码优化**：重复逻辑、过长函数、性能（SQL/网络/UI 渲染）、死代码、调试日志清理
  4. **目录架构完善**：data/ 使用情况；modules 是否需子包拆分；P1/P9/P11 实施后的架构影响
- **交付物**：审计报告 + 问题清单（按严重度）+ 优化建议
- **触发条件**：与 P1/P9/P11 评估合并排期
