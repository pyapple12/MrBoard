...
### P23. 契约层（结构键名）能否完全外置剥离的讨论（设计议题）

- **状态**：📌 待讨论（2026-08-13 用户提出，留待后续理清并想解决方案）
- **来源**：2026-08-13 用户："任何硬编码都不能出现在 py 文件中，这样才健康" + "契约层为什么不能剥离？"
- **讨论结论（截至 2026-08-13）**：
  - 硬编码分四层：值层（可调参数/文案，已外置✅）/ 契约层（结构键名如 COLUMN_IDS/DIMENSIONS，代码声明）/ 逻辑层（SQL/正则/计算，留代码）/ 语法层（import/调用，不可外置）
  - 契约层不可剥离的三重论证：① 校验悖论——期望标准必须独立于实际，配置不能自证缺失；② 语义绑定——键名是取值函数句柄（COLUMN_IDS 的 id 对应 row 提取/格式化逻辑），外置后 switch 分支仍在代码；③ 信息论——接口标识符必须双方声明（类比数据库 schema/API 字段/函数签名）
  - 可剥离部分：契约的元数据（列顺序/宽度/显示名/默认可见性）——纯数据可外置；机制性替代（注册表模式）只是换形式，键集合仍在代码
  - 健康标准定案（暂行）：值层 100% 外置 + 契约层显式声明（配导入期校验 B0.6/C0.8）+ 逻辑层留代码
- **待续讨论点**：
  - theme/view 扩展时是否引入 view/profile 配置层（列配置集独立于 theme）
  - 契约层"显式声明"可否用注册表/生成器优化（构建期 AST 扫描生成契约文件）——评估收益与复杂度
  - 是否将"契约层豁免写进 AGENTS.md"（避免每轮审计重复讨论）
- **触发条件**：用户发起续谈；或 theme/view 功能立项时

### P24. notify 三级兜底链去留（过度防御评估，2026-08-13 记录）

- **状态**：📌 待评估（H0.8 契约落地后评估）
- **来源**：2026-08-13 豁免盘点讨论——选项 B（notify 契约）定案后，三级链职责收窄为"只兜坏模板"，提出"是否彻底删除这条过度防御性的兜底链"
- **背景**（链的历史——两轮审计防护叠加产物）：
  - B0.7（第 8 轮 A008 P3）：模板 `.format` 无防护，未知占位符 KeyError 逃逸 → 加第一层 try + 回退固定文案
  - C0.2（第 9 轮 A009 P2，实测）：B0.7 不完整——`'{used'`/`'{used!q}'` 抛 ValueError、`'{}'` 抛 IndexError 全部逃逸 + fallback 无二次保护 → 扩为 except 全类型 + fallback 二次 try + 纯静态拼接第三层
  - H0.8（当前批次）：键存在性入契约（删键导入期报错），链的职责从"兜键缺失 + 兜坏模板"收窄为"只兜坏模板"
- **现状（H0.8 后）**：三层结构 try(主模板) → try(fallback) → 静态拼接；契约保证键存在；剩余兜底对象 = `.format` 坏模板（KeyError 坏占位符/ValueError 坏格式/IndexError 空括号）
- **重要新证据（2026-08-13 H0 验收发现）**：**坏模板在导入期就被占位符校验拦截**（main_window.py:274，C0.8/D0.5 的 _TEMPLATE_MAP 校验——`{used` 坏格式、`{unknown}` 未知占位符均导入期抛 RuntimeError）——三级链的运行时兜底对象**近乎不存在**，仅剩校验白名单内的合法占位符组合（如 `{used!q}` 转换格式符）理论可达；此发现**显著强化选项 X 论据**（三层链 ≈ 不可达死防御）
- **评估点**：
  - **项目语境**：ui.json 随包发布（正常用户不手改），坏模板仅"手改配置"可达——失败概率极低
  - **静默降级哲学冲突**：坏模板当前静默降级（fallback/静态拼接），掩盖配置错误（C0.1 教训：静默错误最危险）；契约已接管删键，坏模板仍是"静默区"
  - **选项 X（删除三层 → 单层）**：`try: format(主模板) except (KeyError, ValueError, IndexError): 静态拼接`——fallback 模板与主模板同质（同为手改可坏配置），第二层价值存疑；删除后代码 -8 行，坏模板场景用户仍收到静态拼接通知（信息不丢）
  - **选项 Y（保留现状）**：防御纵深——fallback 是"精心维护的备选文案"，主模板坏时用备选更体面；极端场景仍给用户正常格式通知
  - **与 H0.8 的耦合**：若大概率走选项 X，fallback 键的契约（H0.8 已计划加）可先不加，避免"先加后删"（H0.8 执行时需同步评估）
- **触发条件**：H 批次完成后，观察 H0.8 落地后链的实际职责与调用频率再定
