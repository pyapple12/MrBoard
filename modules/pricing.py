...
#   _price_line()：单费率行成本 = count/1e6 × price（price 为 None 计 0，
#     G3.1 修正主语；6A.3 O2 抽取，estimate_cost 四段共用）
#   _rate_from_raw()：原始定价行 → RateInfo 弹性构建（input/output 缺字段按 0、
#     cache 缺省 None 按无折扣，G3.1 修正；内置/缓存/本地覆盖三来源共用，B4）
